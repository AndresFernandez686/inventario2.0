    ...
    for i, categoria in enumerate(inventario.keys()):
        with tabs[i]:
            ...
            if categoria == "Por Kilos":
                ...
                for n in range(num_baldes + 1, 21):
                ...
                for n in range(1, num_baldes + 1):
                    ...
                    if key_balde not in st.session_state:
                        valor_guardado = None
                        # Inicializa con "Vacío" si no hay historial para esa fecha
                        if isinstance(productos[producto_seleccionado], list) and len(productos[producto_seleccionado]) >= n:
                            valor_guardado = productos[producto_seleccionado][n-1].get("estado") if isinstance(productos[producto_seleccionado][n-1], dict) else "Vacío"
                ...
                if st.button(
                    ...
                    # Guardar por fecha: cada balde como diccionario {fecha, estado}
                    registros_fecha = [{"fecha": str(fecha_carga), "estado": estado} for estado in estados_baldes]
                    productos[producto_seleccionado] = registros_fecha

                    guardar_inventario(inventario)
                    guardar_historial(
                        fecha_carga, usuario, categoria, producto_seleccionado, registros_fecha, modo_actualizacion
                    )
                    st.success(f"Actualizado. Estado actual: {', '.join(estados_baldes)}")

                st.write("Inventario actual por fecha:")
                for p, c in productos.items():
                    if isinstance(c, list) and c:
                        estados_por_fecha = [f"{r['estado']} ({r['fecha']})" for r in c]
                        st.write(f"- {p}: {', '.join(estados_por_fecha)}")
                    else:
                        st.write(f"- {p}: Vacío")
